...
public class ArtGallery : MonoBehaviour
{
    ...
    public float wheelScrollSpeed = 100f; // Aumentado significativamente
    ...
    public void LogLayoutDebugInfo()
    {
        ...
        // Info del primer item (si existe)
        if (instantiatedItems.Count > 0)
        {
            GameObject firstItem = instantiatedItems[0];
            RectTransform itemRect = firstItem.GetComponent<RectTransform>();
            Debug.Log($"Primer Item Size: {itemRect.rect.size}");
        }

        Debug.Log($"Total Items: {instantiatedItems.Count}");
    }
    ...
    IEnumerator ForceFixLayout()
    {
        ...
        // Asegurar que el ScrollRect tenga el content asignado
        if (scrollRect != null && contentRect != null)
        {
            scrollRect.content = contentRect;
        }
